  ...
  return (
    <div id="notes-content" className={`prose max-w-none ${theme === 'dark' ? 'prose-invert dark:prose-dark text-gray-100' : 'text-gray-900'} text-lg`}>
      <ReactMarkdown
        ...
        components={{
          h1: ({ children, ...props }) =>
            <h1 className={`text-3xl font-bold relative p-2 mb-2 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`} {...props}>{children}</h1>,
          h2: ({ children, ...props }) =>
            <h2 className={`text-2xl font-semibold relative p-2 mb-2 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`} {...props}>{children}</h2>,
          h3: ({ children, ...props }) =>
            <h3 className={`text-xl font-medium relative p-2 mb-2 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`} {...props}>{children}</h3>,
          p: ({ children, ...props }) =>
